        ...
        .then(data => {
            ...
            data.forEach(tx => {
                ...
                row.innerHTML = `
                    ...
                    <td class="px-4 py-2">
                        <div class="icon-actions">
                            <i class="bx bxs-edit-alt icon edit-icon" onclick="editTuyenXe('${tx.IDTuyenXe}')"></i>
                            <i class="bx bxs-trash icon delete-icon" onclick="deleteTuyenXe('${tx.IDTuyenXe}')"></i>
                        </div>
                    </td>
                `;
        ...
        .then(data => {
            ...
            data.forEach(item => {
                ...
                row.innerHTML = `
                    ...
                    <td class="px-4 py-2">
                        <div class="icon-actions">
                            <i class="bx bxs-edit-alt icon edit-icon" onclick="editChuyenXe('${item.IDChuyen}')"></i>
                            <i class="bx bxs-trash icon delete-icon" onclick="deleteChuyenXe('${item.IDChuyen}')"></i>
                        </div>
                    </td>
                `;
                tableBody.appendChild(row);
            ...
            data.forEach(cx => {
                const row = `
                    <tr>
                        ...
                        <td class="px-4 py-2">
                        <div class="icon-actions">
                            <i class="bx bxs-edit-alt icon edit-icon" onclick="editChuyenXe('${cx.IDChuyen}')"></i>
                            <i class="bx bxs-trash icon delete-icon" onclick="deleteChuyenXe('${cx.IDChuyen}')"></i>
                        </div>
                    </td>
                    </tr>
                `;
...
function searchKhachHang() {
    const tenKhach = document.getElementById("search-khach").value.trim();
    if (!tenKhach) {
        alert("Vui lòng nhập tên khách hàng cần tìm!");
        return;
    }

    fetch(`/khach/name/${encodeURIComponent(tenKhach)}`)
        .then(res => {
